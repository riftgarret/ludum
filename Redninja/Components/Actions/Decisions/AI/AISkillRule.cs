	...
	public class AISkillRule : AIRuleBase
	{
		...
		public override IBattleAction GenerateAction(IBattleEntity source, IDecisionHelper decisionHelper)
		{
			...
			foreach (ISkill skill in availableSkills)
			{
				...
				// found!				
				while (TryFindTarget(targetMeta, source, decisionHelper.EntityModel, out ISelectedTarget selectedTarget))
				{
		...
		internal bool TryFindTarget(ITargetingState meta, IBattleEntity source, IBattleModel bem, out ISelectedTarget selectedTarget)
		{
			// filter targets
		...
		internal IEnumerable<IBattleEntity> FilterTargets(ITargetingRule targetingRule, IBattleEntity source, IBattleModel bem)
		{
